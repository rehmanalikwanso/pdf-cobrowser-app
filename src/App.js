...
import PdfViewerComponent from './PdfViwerDoc';
...
function CobrowseableDocument({ numPages, onDocumentLoadSuccess }) {
  ...
  return (<>
    ...
    <Document file="/Credit-Report-Authorization-Form.pdf" onLoadSuccess={onDocumentLoadSuccess}>
      {Array.from(
        new Array(numPages),
        (el, index) => (
          <Page
...
CobrowseIO.capabilities = ['cursor', 'drawing', 'full_device', 'laser', 'scroll', 'keypress'];
CobrowseIO.ignoredViews = ['#new_unique'];
function App() {
